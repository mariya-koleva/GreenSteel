...
def run_simulation(electrolyzer_rating=None, plant_size=None, verbose=False, show_plots=False, save_plots=False, use_profast=True, storage_type=None, incentive_option=1, plant_design_scenario=1, output_level=1, grid_connection=None):
    ...
    filename_floris_config = "../../input/floris/floris_input_x_20MW_base" + ".yaml"
    ...
    def energy_internals(hopp_results=hopp_results, hopp_site=hopp_site, hopp_technologies=hopp_technologies, hopp_scenario=hopp_scenario, hopp_h2_args=hopp_h2_args, orbit_project=orbit_project, design_scenario=design_scenario, plant_config=plant_config, turbine_config=turbine_config, wind_resource=wind_resource, floris_config=floris_config, electrolyzer_rating=electrolyzer_rating, plant_size=plant_size, verbose=verbose, show_plots=show_plots, save_plots=save_plots, use_profast=use_profast, storage_type=storage_type, incentive_option=incentive_option, plant_design_scenario=plant_design_scenario, output_level=output_level, solver=True, power_for_peripherals_kw_in=0.0, breakdown=False):
        ...
        # print("\nwind kw", sum(hopp_results["combined_pv_wind_power_production_hopp"])/(365*24))
        # print("electrolysis kw ", sum(remaining_power_profile_in)/(365*24))
        # print("dist. peripheral kw ", distributed_peripheral_power)
        # slush = sum(hopp_results["combined_pv_wind_power_production_hopp"])/(365*24) - sum(remaining_power_profile_in)/(365*24) - power_for_peripherals_kw_in
        # print("slush ", slush)
        # print("min ", np.asarray(remaining_power_profile_in).min())
        # print("min ", np.asarray(remaining_power_profile_in).max())

        # residual = sum(hopp_results["combined_pv_wind_power_production_hopp"]) - sum(remaining_power_profile_in)
        # if  < power_for_peripherals_kw_in*365*24:
        #     high_count = sum(remaining_power_profile_in >= power_for_peripherals_kw_in)
        #     remainder = (sum(hopp_results["combined_pv_wind_power_production_hopp"]) - sum(remaining_power_profile_in)) / high_count
        #     remaining_power_profile_in[remaining_power_profile_in>power_for_peripherals_kw_in] -= remainder
        #     print("\nREMAINDER kwh: ", remainder)
        #     print("high count", high_count)
        #     print("remainder kw ", remainder*high_count/(365*24))
        #     print("diff ", sum(hopp_results["combined_pv_wind_power_production_hopp"]) - sum(remaining_power_profile_in))
...
if __name__ == "__main__":
    # run_design_options()
    # run_simulation(verbose=True, show_plots=True, save_plots=False,  use_profast=True, incentive_option=1, plant_design_scenario=1)
    # run_storage_options()
    # for i in range(1,8):
