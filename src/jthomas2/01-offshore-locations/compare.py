...
# from examples.H2_Analysis.hopp_for_h2 import hopp_for_h2; use_floris = False
...
def convert_layout_from_floris_for_orbit(turbine_x, turbine_y, save_config=False):
    ...
    substation_dict = {
                ...
                'string': 0 , # can be left empty
                'order': 0, # can be left empty
                'cable_length': 0,
                'bury_speed': 0
                }
...
def get_inputs(turbine_model="osw_18MW", verbose=False, show_plots=False, save_plots=False):
    ...
    plant_config = orbit.load_config("../../input/plant/orbit-config-"+turbine_model+".yaml")
    ...
    # plt.plot(turbine_config["power_thrust_table"]["wind_speed"], turbine_config["power_thrust_table"]["power"])
    # plt.show()
    # load floris inputs
    ...
    # plant_config = orbit.core.library.extract_library_specs("config", "osw_cable_layout")
    ...
    if show_plots or save_plots:
        ...
        # print(dir(wind_resource))
        # print(size(wind_resource.data["data"]))
    print("\n")
...
def run_orbit(plant_config, verbose=False, weather=None):
    ...
    if verbose:
        ...
        # cable_keys = project.phases["ElectricalDesign"].total_cable_length_by_type.keys()
        # print("key ", (key for key in cable_keys))
        # print("Export cable material CAPEX: %.2f M USD" %(project.phases["ElectricalDesign"].total_cable_length_by_type[cable_keys[0]]*430000*1E-6))
        print("\n")
...
def setup_hopp(plant_config, turbine_config, wind_resource, orbit_project, floris_config, show_plots=False, save_plots=False):
    ...
    # print(len(orbit_project.phases["ArraySystemDesign"].turbines_x.flatten()))
    ...
    if use_floris: # TODO check
        ...
        #     # ax.scatter(floris_config["farm"]["layout_x"]/turbine_config["rotor_diameter"], floris_config["farm"]["layout_y"]/turbine_config["rotor_diameter"])
        #     for i in range(0,len(floris_config["farm"]["layout_x"])):
        #         turb = patches.Circle((floris_config["farm"]["layout_x"][i], floris_config["farm"]["layout_y"][i]), radius=turbine_config["rotor_diameter"]/2.0, color="k", fill=False)
        #         ax.add_patch(turb)
        #     ax.set(xlim=[min(floris_config["farm"]["layout_x"])-200, max(floris_config["farm"]["layout_x"]+200)],
        #         ylim=[min(floris_config["farm"]["layout_y"])-200, max(floris_config["farm"]["layout_y"])+200])
        #     ax.set(aspect="equal")
        #     ax.axis('off')
        #     if save_plots:
        #         plt.savefig("turbine_layout.pdf", transparent=True)
        #     if show_plots:
        #         plt.show()
...
def run_hopp(hopp_site, hopp_technologies, hopp_scenario, hopp_h2_args, verbose=False):
    ...
    # print("HYBRID PLANT")
    # print(dir(hybrid_plant.wind._financial_model.Outputs))
    # # print(len(hybrid_plant.wind._financial_model.Outputs.revenue_gen))
    # # print(dir(hybrid_plant.wind._financial_model.Outputs.cf_annual_costs))
    # # print(hybrid_plant.wind._financial_model.Outputs.cf_disbursement_debtservice)
    # # print(hybrid_plant.wind._financial_model.Outputs.cf_disbursement_equip1)
    # # print(hybrid_plant.wind._financial_model.Outputs.cf_disbursement_equip2)
    # # print(hybrid_plant.wind._financial_model.Outputs.cf_disbursement_equip3)
    # # print(hybrid_plant.wind._financial_model.Outputs.cf_disbursement_om)
    # a = np.asarray(hybrid_plant.wind._financial_model.Outputs.cf_pretax_cashflow)
    # b = np.asarray(hybrid_plant.wind._financial_model.Outputs.cf_annual_costs)
    # c = np.asarray(hybrid_plant.wind._financial_model.Outputs.cf_project_return_pretax)
    # print(a)
    # print(b)
    # print(abs(a)-abs(b))
    # print(c)
    ...
    if verbose:
        ...
        # print("hybrid_plant: ", vars(hybrid_plant.grid.site))
        # print("elec sold: ", hybrid_plant.grid.total_revenue, hybrid_plant.capacity_price)
...
def run_electrolyzer_physics(hopp_results, hopp_scenario, hopp_h2_args, plant_config, wind_resource, design_scenario, show_plots=False, save_plots=False,  verbose=False):
    ...
    if save_plots or show_plots:
        ...
        # fig, ax = plt.subplots(1)
        # ax.plot(electrical_generation_timeseries)
        # if save_plots:
        #     plt.savefig("figures/electrical_generation/")
        # if show_plots:
        #     plt.show()
...
def run_electrolyzer_cost(electrolyzer_physics_results, hopp_scenario, plant_config, design_scenario, verbose=False):
    ...
    #     # cf_h2_annuals, electrolyzer_total_capital_cost, electrolyzer_OM_cost, electrolyzer_capex_kw, time_between_replacement, h2_tax_credit, h2_itc = \
    #     #     basic_H2_cost_model(plant_config["electrolyzer"]["electrolyzer_capex"], plant_config["electrolyzer"]["time_between_replacement"], electrolyzer_size_mw, useful_life, atb_year,
    #     #     electrical_generation_timeseries, H2_Results['hydrogen_annual_output'], hopp_scenario['H2 PTC'], hopp_scenario['Wind ITC'], include_refurb_in_opex=False)
    
    #     # # Singlitico
    #     P_elec =  electrolyzer_size_mw*1E-3 # [GW]
    #     RC_elec = plant_config["electrolyzer"]["electrolyzer_capex"] # [USD/kW]

    #     pem_offshore = PEMCostsSingliticoModel(elec_location=0)

    #     electrolyzer_capital_cost, electrolyzer_om_cost = pem_offshore.run(P_elec, RC_elec)

    #     electrolyzer_total_capital_cost = electrolyzer_capital_cost*1E6 # convert from M USD to USD
    #     electrolyzer_OM_cost = electrolyzer_om_cost*1E6 # convert from M USD to USD
    ...
    #     # electrolyzer_total_capital_cost = pem_offshore.calc_capex(P_elec, RC_elec)*1E6 # returns in M USD
    #     # electrolyzer_OM_cost = pem_offshore.calc_opex(P_elec, RC_elec)*1E6 # returns in M USD
    ...
    #     RC_elec = plant_config["electrolyzer"]["electrolyzer_capex"] # [USD/kW]
    #     nturbines = plant_config["plant"]["num_turbines"]
    #     in_turb_elec_rating = plant_config["electrolyzer"]["rating"]*1E-3/nturbines #[GW] TODO consider running individual PEM performance simulations for each turbine
        
    #     pem_offshore = PEMCostsSingliticoModel(elec_location=1)

    #     electrolyzer_capital_cost_per_turbine, electrolyzer_om_cost_per_turbine = pem_offshore.run(in_turb_elec_rating, RC_elec)

    #     # electrolyzer_capital_cost_per_turbine = pem_offshore.calc_capex(in_turb_elec_rating, RC_elec) # returns in MUSD
    #     # electrolyzer_om_cost_per_turbine = pem_offshore.calc_opex(P_elec=in_turb_elec_rating, capex_elec=electrolyzer_capital_cost_per_turbine) # returns in M USD
        
    #     electrolyzer_total_capital_cost = nturbines*electrolyzer_capital_cost_per_turbine*1E6 # convert from M USD to USD
    #     electrolyzer_OM_cost = nturbines*electrolyzer_om_cost_per_turbine*1E6 # convert from M USD to USD
    # else:
    #     raise ValueError("Invalid hydrogen production location. Must be one of ['onshore', 'platform', 'turbine']")
...
def run_h2_transport_compressor(plant_config, electrolyzer_physics_results, design_scenario, verbose=False):
    ...
    if design_scenario["transportation"] == "pipeline" or (design_scenario["h2_storage"] != "onshore" and design_scenario["h2_location"] == "onshore"):
        ...
        ################ old compressor model
        # flow_rate_kg_per_hr = max(electrolyzer_physics_results["H2_Results"]["hydrogen_hourly_production"]) # kg/hr
        # number_of_compressors = np.ceil(flow_rate_kg_per_hr/plant_config["h2_transport_compressor"]["flow_rate"])
        # # set up input dict based on plant_config file
        # in_dict = dict()
        # in_dict['flow_rate_kg_hr'] = flow_rate/number_of_compressors # flow rate in kg/hr per compressor
        # in_dict['P_outlet'] = plant_config["h2_transport_compressor"]["output_pressure"] # bar
        # in_dict['compressor_rating_kWe'] = plant_config["h2_transport_compressor"]["energy_rating"] # kWe
        # in_dict['mean_time_between_failure'] = plant_config["h2_transport_compressor"]["mean_days_between_failures"] # days
        # in_dict['total_hydrogen_throughput'] = electrolyzer_physics_results["H2_Results"]["hydrogen_annual_output"] # kg/yr
        # in_dict["number_of_compressors"] = number_of_compressors 
        # in_dict['useful_life'] = plant_config["h2_transport_compressor"]["useful_life"] # years
        # in_dict['plant_life'] = plant_config["project_parameters"]["project_lifetime"] # years
        
        # # set up results dict
        # h2_transport_compressor_results = dict()

        # # run compressor model
        # compressor = Compressor(in_dict, h2_transport_compressor_results)
        # compressor.compressor_power()
...
def run_h2_storage(plant_config, turbine_config, electrolyzer_physics_results, design_scenario, verbose=False):
    ...
    ###################################
        ...
        h2_capacity = round(round(storage_hours*storage_max_fill_rate*scale)/scale)
    ...
    if plant_config["h2_storage"]["type"] == "none" or design_scenario["h2_storage"] == "none":
        ...
        ### discounting done in run_capex
        # # on turbine model gives results for a single turbine in 2006 dollars, so we adjust here for inflation and the number of turbines
        # h2_storage_results["storage_capex"] = -nturbines*npf.fv(plant_config["finance_parameters"]["general_inflation"], plant_config["atb_year"]-2003, 0.0, h2_storage.get_capex())
        # h2_storage_results["storage_opex"] = -nturbines*npf.fv(plant_config["finance_parameters"]["general_inflation"], plant_config["atb_year"]-2003, 0.0, h2_storage.get_opex())
        ...
        # storage_input['storage_duration_hrs'] = storage_hours
        # storage_input['flow_rate_kg_hr'] = np.max(electrolyzer_physics_results["H2_Results"]["hydrogen_hourly_production"])       #[kg-H2/hr]
...
def run_capex(hopp_results, orbit_project, electrolyzer_cost_results, h2_pipe_array_results, h2_transport_compressor_results, h2_transport_pipe_results, h2_storage_results, plant_config, design_scenario, desal_results, platform_results, verbose=False):
    ...
    # I think hopp installed costs is just the wind? why is this coming from grid?
    # [?] why is grid used even when supposedly not grid connected?
    # it looks like grid._financial_model is just the SAM financial model, that just sums the inputs provided

    # access specific inputs
    ## energy plant capex [?] why is grid used here?
    ## was using this (Jared)
    # hybrid_plant = hopp_results["hybrid_plant"]
    # total_hopp_installed_cost = hybrid_plant.grid._financial_model.SystemCosts.total_installed_cost
    ## now using this (Jared)
    total_wind_installed_costs_with_export = orbit_project.total_capex

    ## electricity export system
    ...
    #TODO make sure CAPEX is correct, and use the parts of CAPEX that make sense (elec. gen, export, etc)
    ## desal capex
    ...
    for key in capex_breakdown.keys():
        ...
        # print("key: ", key, " capex base: ", capex_base, " capex discounted: ", capex_breakdown[key], " year: ", cost_year)
    # calculate the total CAPEX of the installed system
    # total_system_installed_cost_hvdc = total_hopp_installed_cost + desal_capex + total_export_system_cost + electrolyzer_total_capital_cost + compressor_capex + h2_storage_capex
    # total_system_installed_cost_hvdc = total_wind_cost_no_export + desal_capex + total_export_system_cost + electrolyzer_total_capital_cost + compressor_capex + h2_storage_capex
    ...
    if verbose:
        ...
        # print("wind: ", "$%.2f" % (total_wind_cost_no_export*1E-6), " M") #good
        # print("desal: ", "$%.2f" % (desal_capex*1E-6), " M") # good for no desal case
        # print("export: ", "$%.2f" % (total_export_system_cost*1E-6), " M") # high? 4x? (should be closer to $160 M, $85 M for cable cost)
        # print("electrolyzer: ", "$%.2f" % (electrolyzer_total_capital_cost*1E-6), " M") # seems about 2x what I would expect, but still in the right order of magnitude
        # print("compressor: ", "$%.2f" % (compressor_capex*1E-6), " M") # 5 M? seems a little high to me
        # print("h2 storage: ", "$%.2f" % (h2_storage_capex*1E-6), " M") # 10 M? seems like this should be closer to 1 M?
...
def run_opex(hopp_results, orbit_project, electrolyzer_cost_results, h2_pipe_array_results, h2_transport_compressor_results, h2_transport_pipe_results, h2_storage_results, plant_config, desal_results, platform_results, verbose=False, total_export_system_cost=0):
    ...
    # electricity transport OPEX
    # site_df = orbit_project.capex_breakdown # pull from ORBIT
    # print(site_df)
    # wind_cost_kw, wind_om_cost_kw, total_export_system_cost, total_export_om_cost = hopp_tools.set_export_financials(plant_config["plant"]["capacity"], plant_config["project_parameters"]["turbine_capex"], plant_config["project_parameters"]["opex_rate"], plant_config["project_parameters"]["project_lifetime"], site_df)
    # total_export_om_cost =

    ############ Rough annual OpEx Estimation from OSW
    # https://www.sciencedirect.com/science/article/pii/S0360319921009137?via%3Dihub = 0.5% CapEx per lifetime for offshore cables 
    total_export_om_cost = 0.0 # TODO check how this should be handled. OPEX is lumped in ORBIT, so leaving at 0.0 for individual component
    # total_export_om_cost = 0.5/100 * total_export_system_cost / plant_config["project_parameters"]["project_lifetime"] # $US total (assume 30 year lifetime))
    ########### 
    ...
    if verbose:
        ...
        # print("wind: ", "$%.2f" % (annual_operating_cost_wind*1E-6), " M") #
        # print("desal: ", "$%.2f" % ( annual_operating_cost_desal*1E-6), " M") # 
        # print("export: ", "$%.2f" % (total_export_om_cost*1E-6), " M") # 
        # print("electrolyzer: ", "$%.2f" % (annual_operating_cost_h2*1E-6), " M") # 
        # print("compressor: ", "$%.2f" % (compressor_opex*1E-6), " M")
        # print("h2_storage: ","$%.2f" % (storage_opex*1E-6), " M") # 
...
def plot_policy_storage_design_options(colors=None, normalized=False):
    ...
    plt.savefig("figures/aggregate/design-policy-lcoh-%s.png" %(normname), transparent=True)
    plt.tight_layout()
    plt.show()
...
def run_design_options(verbose=False, show_plots=False, save_plots=False,  incentive_option=1):
    ...
    design_options = [1,7] #range(1,8) # 8
    scenario_lcoh = []
    scenario_lcoe = []
