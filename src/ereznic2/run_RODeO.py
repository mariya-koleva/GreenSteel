...
def run_RODeO(atb_year,site_name,policy_option,policy,wind_size_mw,solar_size_mw,electrolyzer_size_mw,\
                    energy_to_electrolyzer,electrolyzer_energy_kWh_per_kg_design,hybrid_plant,capex_wind_installed,capex_solar_installed,capex_battery_installed,\
                    ...
                    gams_locations_rodeo_version,rodeo_output_dir,water_cost,elec_efficiency_kWhprkg,gen_inflation,property_tax_insurance,total_income_tax_rate_combined,capitalgains_tax_rate_combined,\
     ...
     electrical_generation_timeseries = electrical_generation_timeseries/electrolyzer_size_mw
     ...
     ren_profile_name = 'ren_profile_'+str(atb_year) + '_'+site_name
     electrical_generation_timeseries_df.to_csv("H2_Analysis/RODeO_files/Data_files/TXT_files/Ren_profile/" + ren_profile_name + '.csv',sep = ',')
    ...
    #  # Storage costs as a function of location. Eventuall change this to specify type of storage to RODeO
     if site_name == 'IA':
         h2_storage_cost_USDperkg = model_year_CEPCI/equation_year_CEPCI*521.34
     elif site_name == 'TX':
         h2_storage_cost_USDperkg = model_year_CEPCI/equation_year_CEPCI*17.04
     elif site_name == 'IN':
         h2_storage_cost_USDperkg = model_year_CEPCI/equation_year_CEPCI*521.34
     elif site_name == 'MS':
         h2_storage_cost_USDperkg = model_year_CEPCI/equation_year_CEPCI*17.04
     elif site_name == 'MN':
         h2_storage_cost_USDperkg = model_year_CEPCI/equation_year_CEPCI*42.42
         
     # O&M costs
     # https://www.sciencedirect.com/science/article/pii/S2542435121003068
     time_between_replacement = 30000 # EVentually connect to HOPP output
     ...
     amortized_refurbish_cost = (capex_electrolyzer_overnight/electrolyzer_size_mw*stack_replacement_cost)\
             *max(((plant_life*8760*elec_cf)/time_between_replacement-1),0)/plant_life/8760/elec_cf

     total_variable_OM = variable_OM+amortized_refurbish_cost
     
     # Wind and solar costs
     if grid_connection_scenario != 'grid-only':
        hybrid_installed_cost_perMW = (capex_wind_installed + capex_solar_installed + capex_battery_installed)/system_rating_mw
        hybrid_om_cost_MW = (fixed_cost_wind + fixed_cost_solar + fixed_cost_battery)/system_rating_mw
     else:
        hybrid_installed_cost_perMW = 0
        hybrid_om_cost_MW = 0

     # Define electrolyzer capex, fixed opex, and energy consumption (if not pulling from external data)
     electrolyzer_capex_USD_per_MW = capex_electrolyzer_overnight/electrolyzer_size_mw
     electrolyzer_fixed_opex_USD_per_MW_year = fixed_cost_electrolysis_total/electrolyzer_size_mw
    ...
    # Define dealination conversion factors
     ...
     desal_sys_size_m3_hr = electrolyzer_size_mw*1000/elec_efficiency_kWhprkg *m3_water_per_kg_h2
    ...
    # electrolysis_desal_total_energy_consumption = electrolyzer_energy_kWh_per_kg + desal_energy_kWh_per_kg_H2
     ...
     # Desal capex and opex in $/MW
    #  desal_capex_per_mw = capex_desal/electrolyzer_size_mw
    #  desal_opex_per_mw = opex_desal/electrolyzer_size_mw
     
     electrolysis_desal_total_energy_consumption = elec_efficiency_kWhprkg + desal_energy_kWh_per_kg_H2
     ...
     inflation_rate = gen_inflation
     equity_percentage = 1/(debt_equity_ratio_combined+1)#  42/100
     ...
     # Set up batch file
     dir0 = "..\\..\\..\\RODeO\\"
     dir1 = 'H2_Analysis\\RODeO_files\\Data_files\\TXT_files\\'
    ...
    # txt1 = '"C:\\GAMS\\win64\\24.8\\gams.exe" ..\\RODeO\\Storage_dispatch_SCS license=C:\\GAMS\\win64\\24.8\\gamslice.txt'
     ...
     scenario_name = str(atb_year)+'_'+ site_name +'_'+policy_option.replace(' ','-') + '_' + grid_string
     ...
     energy_price = ' --energy_purchase_price_inst=Elec_prices\\Elec_purch_price_MWh_MC100by35_'+grid_price_scenario+'_'+site_name+'_'+str(grid_year)
     ...
     roe_inst = ' --roe_instance=' + str(round(nominal_roe_combined,4))
     debt_interest_inst = ' --debt_interest_instance=' + str(round(nominal_interest_combined,4))
     cftr_inst = ' --cftr_instance='+str(round(total_income_tax_rate_combined,4))
     ...
     if grid_connection_scenario == 'grid-only':
         ...
         ren_fom = ' --renew_FOM_cost_inst='+str(round(hybrid_om_cost_MW))
     ...
     ren_itc = ' --itc_ren_inst='+str(round(policy['Wind ITC'],5))
     stor_itc = ' --itc_stor_inst='+str(round(policy['Storage ITC'],5))
    ...
    #  os.startfile('..\\..\\..\\RODeO\\Output_batch.bat')  
    ...
    #  print(temp)  
     ...
     storage_duration_hr = RODeO_results_summary_dict['storage capacity (MWh)']/RODeO_results_summary_dict['input efficiency (%)']/electrolyzer_size_mw
     ...
     hydrogen_hourly_inputs_RODeO = pd.read_csv(dirout+'\\Storage_dispatch_inputs_'+scenario_name + '.csv',index_col = None,header = 30)
     ...
     return(scenario_name,lcoh,electrolyzer_capacity_factor,storage_duration_hr,storage_capacity_kg,hydrogen_annual_production,water_consumption_hourly_array,RODeO_results_summary_dict,hydrogen_hourly_inputs_RODeO,hydrogen_hourly_results_RODeO,electrical_generation_timeseries,electrolyzer_capex_USD_per_MW,h2_storage_cost_USDperkg)